...
class MIBasicModel(Model):
    ...
    def getSignals(self, window, idx=0):
        ...
        predictions = self.assembly.get_predictions_with_raw_data(window, self.trainingRunId, debug=self.debug)
        if not predictions.empty:
            predictionSignals = mlutils.toTradeSignals(mlutils.onehot(predictions[["y_pred0"]].values), self.threshold)
            predictionSignals = pd.DataFrame(np.array([predictionSignals, predictionSignals]).T, index=predictions.index, columns=["bar", "gap"])
            if (self.barOnly):
